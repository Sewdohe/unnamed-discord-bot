...
export function warnCommand(ctx: PluginContext<ModConfig>, api: CoreUtilsAPI, repo: ModerationRepository): Command {
  return {
    ...
    async execute(interaction: any) {
      ...
      // Create case
      const caseId = await repo.createCase("warn", user.id, user.tag, interaction.user.id, interaction.user.tag, reason);

      // Get warning count
      const warnings = (await repo.getUserCases(user.id)).filter(c => c.type === "warn");
...
export function historyCommand(ctx: PluginContext<ModConfig>, api: CoreUtilsAPI, repo: ModerationRepository): Command {
  return {
    ...
    async execute(interaction: any) {
      const user = interaction.options.getUser("user", true);
      const cases = await repo.getUserCases(user.id);
