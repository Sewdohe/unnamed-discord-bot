# GEMINI.md - Discord Bot Framework

This document provides comprehensive context for the AI agent when working on this Discord bot framework. It covers architecture, conventions, common tasks, and implementation patterns.
...
MongoDB Query builder for constructing safe, parameterized queries. It converts a SQL-like query builder API to MongoDB filter objects, providing a more intuitive way to build complex queries.

### `src/core/repository.ts`

Base repository class providing common CRUD operations for MongoDB. It integrates with the `MongoQueryBuilder` and can be extended to create domain-specific repositories with optional schema validation.

### `src/core/schema.ts`

Defines `createSchemaValidator` using Zod for runtime validation of database records and common schemas (`commonSchemas`) for consistent validation across plugins (e.g., `discordId`, `timestamp`).
...
function myCommand(ctx: PluginContext<PluginConfig>): Command {
  return { /* ... */ };
}
...
-   **Table names**: use `ctx.dbPrefix` + lowercase (`${ctx.dbPrefix}tablename`)
...
async onLoad(ctx) {
  ...
  if (!coreUtils?.api) {
    ctx.logger.error("core-utils plugin not found, database features might be limited.");
    return;
  }
...
MONGODB_URI=mongodb://localhost:27017 # Your MongoDB connection URI
```
...
3.  **Database errors**: Ensure collection name uses `ctx.dbPrefix`, and that indexes are created as needed.
...
-   Always prefix database collection names with `ctx.dbPrefix`
